...
class Ball(Drawable):
    def __init__(self, x: int, y: int, radius: int = 20, color=(255, 10, 0), speed: int = 3, ):
    ...
    def bounce_y_power(self):
        self.color = (self.color[0], self.color[1] + 10 if self.color[1] < 255 else self.color[1], self.color[2],)
        pygame.draw.ellipse(self.surface, self.color, [0, 0, self.width, self.height])
        self.x_speed *= 1.1
        self.y_speed *= 1.1
        self.bounce_y()
...
class Racket(Drawable):
    def __init__(self, x: int, y: int, width: int = 80, height: int = 20, color=(255, 255, 255), max_speed: int = 10, ):
...
class PongGame:
    def __init__(self, width: int, height: int, player1: Type[Player], player2: Type[Player]):
        pygame.init()
    ...
    def run(self):
        while not self.handle_events():
            ...
            self.oponent.act(self.oponent.racket.rect.centerx - self.ball.rect.centerx,
                             self.oponent.racket.rect.centery - self.ball.rect.centery, )
            self.player.act(self.player.racket.rect.centerx - self.ball.rect.centerx,
...
class FuzzyPlayer(Player):
    def __init__(self, racket: Racket, ball: Ball, board: Board):
        ...
        x_dist = fuzz.control.Antecedent(np.arange(-400, 400, 1), 'x_distance')
        y_dist = fuzz.control.Antecedent(np.arange(0, 400, 1), 'y_distance')
        out = fuzz.control.Consequent(np.arange(-400, 400, 1), 'move')
        ...
        rule1 = fuzz.control.Rule(x_dist['average'], out['average'])
        rule2 = fuzz.control.Rule(x_dist['poor'] & y_dist['poor'], out['good'])
        rule3 = fuzz.control.Rule(x_dist['poor'] & y_dist['good'], out['decent'])
        rule4 = fuzz.control.Rule(x_dist['good'] & y_dist['poor'], out['poor'])
        rule5 = fuzz.control.Rule(x_dist['good'] & y_dist['good'], out['mediocre'])

        out_ctrl = fuzz.control.ControlSystem([rule1, rule2, rule3, rule4, rule5])
        self.output_simulation = fuzz.control.ControlSystemSimulation(out_ctrl)

    def racket_calc(self, x_diff, y_diff):
        self.output_simulation.input['x_distance'] = x_diff
        self.output_simulation.input['y_distance'] = y_diff
        self.output_simulation.compute()

        return self.output_simulation.output['move']
    ...
    def make_decision(self, x_diff: int, y_diff: int):
        temp = self.racket_calc(x_diff, y_diff)
        print(f"X = {x_diff}, Y = {y_diff}, Val = {temp}")
        return temp
